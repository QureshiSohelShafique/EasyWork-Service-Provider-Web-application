...
export class ServiceBookingComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.bookingForm.value.date == '' || this.bookingForm.value.time == '') {
      ...
      var endMinutes: number = 0;
      if (this.bookingForm.value.time) {
        endTime = +this.bookingForm.value.time.split(':')[0] + 1;
        endMinutes = +(this.bookingForm.value.time.split(':')[1] ?? 0);
      }
      var end_time: string = endTime.toString() + ':' + endMinutes.toString();
